...
public class Runner {
	...
	public static void main(String[] args) {
		...
		System.out.println("saveTo1PDF - zapisuje wszystkie utworzone dokumenty w PDF");
		System.out.println("saveToManyPDFs - zapisuje utworzone dokumenty w ró¿nych plikach PDF");
		System.out.println("saveToManyTXTs - zapisuje utworzone dokumenty w ró¿nych plikach TXT");
...
//-------------------------- saveTo1PDF -------------------------------------//	
	...
	private static void saveTo1PDF(ArrayList <Dokument> lista) {
		try {
			Document document = new Document();
			...
			document.close();
			System.out.println("Documents have been saved to one PDF file.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//-------------------------- saveToManyPDFs -----------------------------------//
	...
	private static void saveToManyPDFs(ArrayList <Dokument> lista) throws IOException {
		for (int i = 0; i<lista.size(); i++) {
			lista.get(i).saveMeToPDF(i);
			}
		System.out.println("Documents have been saved to multiple PDF files.");
		}
...
//----------------------------- saveToManyTXTs -------------------------------------//
	...
	private static void saveToManyTXTs(ArrayList <Dokument> lista) throws IOException {
		for (int i = 0; i<lista.size(); i++) {
		lista.get(i).saveMeToTXT(i);
		}
		System.out.println("Documents have been saved to multiple TXT files.");
	}


//------------------------------- metoda do Faktury ------------------------------//

	private static void askFaktura(ArrayList <Produkt> produkty, BufferedReader input) throws IOException{
		System.out.println("Podaj nazwê produktu: ");
		String nazwa  = input.readLine();
		System.out.println("Podaj cenê produktu: ");
		String cenaa  = input.readLine();
		double cena = Double.parseDouble(cenaa);
		Produkt mojNowyProdukt = new Produkt(nazwa, cena);
		produkty.add(mojNowyProdukt);
	}
	}
